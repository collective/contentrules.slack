...
def test_utility_is_registered_properly():
    slack_notifier = getUtility(ISlackNotifier)
    assert isinstance(slack_notifier, SlackNotifier) is True


def test_notify_to_webhook_performs_a_post_request(wait_for):
    with HTTPXMock.installed() as requests:
        wait_for(notify_slack("https://hooks.slack.com/services/foo"))
        assert len(requests.posts) == 1
        assert requests.posts[0].get("url") == "https://hooks.slack.com/services/foo"


def test_notify_includes_request_payload(wait_for):
    with HTTPXMock.installed() as requests:
        wait_for(notify_slack("https://hooks.slack.com/services/foo", text="Foo bar"))
...
def test_notify_to_webhook_without_webhook_url_will_use_standard_slack_webhook_url(
    ...
    with HTTPXMock.installed() as requests:
        ...
        assert requests.posts[0].get("url") == "https://hooks.slack.com/services/bar"


def test_do_not_post_if_no_webhook_url_is_available(monkeypatch, portal, wait_for):
...
def test_default_request_parameters(wait_for):
    with HTTPXMock.installed() as requests:
        wait_for(notify_slack("https://hooks.slack.com/services/foo"))
...
def test_override_default_request_parameters_is_possible(wait_for):
    with HTTPXMock.installed() as requests:
        wait_for(
            notify_slack(
                "http://someurl", timeout=5, verify=False, param1="Foo", param2="Bar"
            )
        )
