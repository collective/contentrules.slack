...
def test_invoke_add_View(portal, payload, http_request):
    element = getUtility(IRuleAction, name="plone.actions.Slack")
...
def test_invoke_edit_view(http_request):
    element = getUtility(IRuleAction, name="plone.actions.Slack")
    e = SlackAction()
...
def test_payload(slack_action, folder, doc):
...
def test_payload_with_none_values(slack_action, folder, doc):
...
def test_execute(slack_action, wait_for, folder, doc):
    ex = getMultiAdapter((folder, slack_action, DummyEvent(doc)), IExecutable)
